...
export default {
  ...
  compactNumber(value, decimals) {
    if (value < 1e3) {
      ...
      return `${this.formatString(value / 1e3, decimals)}`;
    } else if (value >= 1e6 && value < 1e9) {
      return `${this.formatString(value / 1e6, decimals)}`;
    } else if (value >= 1e9 && value < 1e12) {
      return `${this.formatString(value / 1e9, decimals)}`;
    } else if (value >= 1e12) {
      return `${this.formatString(value / 1e12, decimals)}`;
    }
  },
};
